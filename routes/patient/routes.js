...
const {isPatient,getUserInfo,isDoctor} = require('../../controllers/auth/authMiddleware');
...
router.post('/search-doctors', isPatient, async (req, res) => {
    try {
        ...
        // If no doctors are available, set a flash message and redirect
        if (doctors.length === 0) {
            req.flash('error', 'Sorry, no doctors are available on the selected date.');
            return res.redirect('/my-doctors');
        }
        ...
        res.render('my-doctors', {
            patient,
            doctors: doctorsWithSpecialty,
            selectedDate,
